...
export default class Player {
    ...
    resize(oldGameWidth, oldGameHeight) {
        // Calculate the player's current center X as a ratio of the old game width
        const xRatio = oldGameWidth > 0 ? (this.x + this.width / 2) / oldGameWidth : 0.5;

        this.width = Math.min(this.game.width * 0.15, 80);
        this.height = this.width;

        // Apply the ratio to the new game width to find the new center X
        ...
        // Recreate the mesh with the new dimensions
        if (this.mesh) {
            this.createMesh();
        }
    }
